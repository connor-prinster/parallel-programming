...
int main(int argc, char** argv) {
	...
	// this is to wipe the file of any existing data
	if(rank == MASTER) {
		ofstream ofs;
	...
	// the starting point of each process is determined here
	int store[interval * WIDTH];
	int fakeEye = start;
	int ctr = 0;

	// This nested loop goes through equal amounts of work for each process
	// but the 'fakeEye' variable declared above and used below pretends
	// to be the 'i' value that would have existed had this been a serial
	// mandelbrot
	...
	if(rank == 0) {
		...
		printToFile(rank, store, interval * WIDTH);
		if(rank != (numProc - 1)) { // make sure it's not the last process
			MPI_Send(&dest, 1, MPI_INT, (rank + 1), 0, MCW);
		}
	}


	// end the timing on the last process holding data (the last one)
	if(rank == (numProc - 1)) {
